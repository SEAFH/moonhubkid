export default function handler(req, res) {
  ...
  // Nếu là Roblox executor → trả về script
  ...
  // Nếu là trình duyệt → trả về giao diện HTML
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-logo {
      ...
      font-family: 'Urbanist', sans-serif; font-size: 1.1rem; font-weight: 800;
      color: var(--text); text-decoration: none;
    }
    .moon-icon {
      width: 28px; height: 28px;
      background: linear-gradient(135deg, var(--purple), var(--purple-dim));
      border-radius: 50%; box-shadow: 0 0 14px var(--purple-glow); position: relative; overflow: hidden;
    }
    .moon-icon::after {
    ...
    main {
      ...
      align-items: center; padding: 90px 20px 60px; text-align: center;
    }
    .wordmark {
      font-family: 'Urbanist', sans-serif; font-size: clamp(4.5rem, 11vw, 8rem);
      ...
      filter: drop-shadow(0 0 40px rgba(168, 85, 247, 0.5)); animation: fadeUp 0.7s ease both;
    }
    .wordmark sup { font-size: 0.28em; vertical-align: super; letter-spacing: 0; opacity: 0.85; }
    .tagline {
      margin-top: 22px; font-size: clamp(0.95rem, 2.2vw, 1.2rem);
      color: var(--muted); letter-spacing: 0.01em; animation: fadeUp 0.7s 0.1s ease both;
    }
    #game-name {
      display: inline-block; color: var(--purple-light); font-weight: 500;
      transition: opacity 0.3s, transform 0.3s;
    }
    #game-name.fade-out { opacity: 0; transform: translateY(-6px); }
    .code-box {
      margin-top: 36px; display: flex; align-items: center; gap: 14px;
      background: rgba(20, 16, 35, 0.7); border: 1px solid var(--border); border-radius: 10px;
      padding: 14px 18px 14px 22px; backdrop-filter: blur(10px);
      box-shadow: 0 0 0 1px rgba(168, 85, 247, 0.06), inset 0 1px 0 rgba(255,255,255,0.04);
      animation: fadeUp 0.7s 0.2s ease both; max-width: 560px; width: 100%;
    }
    .code-text {
      font-family: 'DM Mono', monospace; font-size: 0.82rem; color: #c4b5fd;
      white-space: normal; word-break: break-all; flex: 1; text-align: left;
    }
    .code-text .kw { color: #7dd3fc; }
    .code-text .str { color: #a78bfa; }
    .copy-btn {
      background: none; border: 1px solid var(--border); border-radius: 6px;
      width: 32px; height: 32px; cursor: pointer; display: flex; align-items: center;
      justify-content: center; flex-shrink: 0; transition: border-color 0.2s, background 0.2s; color: var(--muted);
    }
    ...
    .btn-row {
      ...
      justify-content: center; animation: fadeUp 0.7s 0.3s ease both;
    }
    .btn-primary {
      display: inline-block; text-decoration: none;
      background: linear-gradient(135deg, var(--purple) 0%, var(--purple-dim) 100%);
      color: #fff; border: none; border-radius: 8px; padding: 13px 30px;
      font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 700; cursor: pointer;
      box-shadow: 0 0 24px rgba(168, 85, 247, 0.5), 0 4px 14px rgba(0,0,0,0.3);
      transition: transform 0.15s, box-shadow 0.15s;
    }
    .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 0 36px rgba(168, 85, 247, 0.7), 0 6px 20px rgba(0,0,0,0.35); }
    .btn-secondary {
      background: transparent; color: var(--text); border: 1px solid rgba(168, 85, 247, 0.4);
      border-radius: 8px; padding: 13px 30px; font-family: 'DM Sans', sans-serif;
    ...
    .keyless-note {
      margin-top: 14px; font-size: 0.82rem; font-weight: 700; color: var(--purple-light);
      letter-spacing: 0.04em; text-transform: lowercase; animation: fadeUp 0.7s 0.35s ease both;
    }
    ...
    .game-card {
      background: rgba(20, 16, 35, 0.6); border: 1px solid var(--border); border-radius: 10px;
      padding: 16px 18px; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; font-weight: 600;
      color: var(--text); backdrop-filter: blur(8px); transition: border-color 0.2s, transform 0.15s, background 0.2s;
    }
    ...
    .faq-btn {
      ...
      background: rgba(20, 16, 35, 0.6); border: 1px solid var(--border); border-radius: 10px;
      padding: 18px 22px; color: var(--text); font-family: 'DM Sans', sans-serif;
      font-size: 0.95rem; font-weight: 700; cursor: pointer; transition: border-color 0.2s, background 0.2s; backdrop-filter: blur(8px);
    }
    ...
    .faq-answer {
      display: grid; grid-template-rows: 0fr; transition: grid-template-rows 0.3s ease, padding 0.3s ease;
      overflow: hidden; font-size: 0.875rem; color: var(--muted);
      border: 1px solid var(--border); border-top: none; border-radius: 0 0 10px 10px;
      background: rgba(14, 10, 25, 0.5); line-height: 1.6; padding: 0 22px;
    }
